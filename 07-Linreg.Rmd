...
# df<-data.frame(norders,app,times)
# write.csv(df, file="norders.csv")

mod<-lm(norders~app)
out<-summary(mod)
```

![](images/linreg2gr.png){width=70%}


We see, that the estimates are ($\color{green}{n=100}$):

\[\hat\alpha=`r round(mod$coefficients[1],4)` \leadsto \text{const}\]

and

\[\hat\beta=`r round(mod$coefficients[2],4)`\leadsto \text{app}.\]
...
![](images/linreg3gr.png){width=70%}

- \(95\%\CI for \(\hat\alpha\) (\(\color{green}{n=100}\)):
)-CI for \(\hat\alpha\) (\(n=100\)):
...
![](images/linreg4gr.png){width=70%}

Since both $p$-values are very small ([scientific notation](https://en.wikipedia.org/wiki/Scientific_notation)), we reject the Null hypothesis of zero coefficients. Of course, we can also use the $t$-values (the $t$-ratio) and check whether they lie in the rejection region instead.
